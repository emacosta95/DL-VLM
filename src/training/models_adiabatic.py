...
class TDDFTadiabaticModel(nn.Module):
    def __init__(
        self,
...
class EnergyReductionXXZXRespect2X(nn.Module):
    def __init__(
        self,
        model: nn.Module,
    ) -> None:
    ...
    def forward(self, x: torch.Tensor, h: torch.Tensor):
        ...
        if x.shape[0] != 1:
            output = self.model(x)  # batch x 2 x size
        else:
        ...
        # sum of zz + x
        z_sp = -1 * torch.sqrt(1 - x**2)
        z_sp_plus = torch.roll(z_sp, dims=-1, shifts=+1)

        f_tot = df - 1 * (z_sp_plus * z_sp) + z_sp  # we assume omega=1
        # longitudinal external term
        e_ext = h[:, 0] * x  # batch x size
        # transverse external term
    ...
    def functional_value(self, z: torch.Tensor):
        if z.shape[0] != 1:
            ...
            f = self.model(z)[:, 0].unsqueeze(0)
        return f


class EnergyReductionXXZ(nn.Module):
    def __init__(
        self,
        model: nn.Module,
    ) -> None:
    ...
    def forward(self, z: torch.Tensor, h: torch.Tensor):
        # single take
        if z.shape[0] != 1:
            df = self.model(z)  # batch x 1 x size
        else:
            df = self.model(z).unsqueeze(0)
        # sum of zz + x
        f_tot = df[:, :]  # + torch.roll(z[:, 0, :], shifts=-1, dims=-1) * z[:, 0, :]
        # longitudinal external term
        e_ext = h * z  # batch x size
        f = (
            -1
            * torch.roll(torch.sqrt(1 - z[:, :] ** 2), shifts=-1, dims=-1)
            * torch.sqrt(1 - z[:, :] ** 2)
            + df
        )
        # transverse external term

        return e_ext.sum(-1).sum(-1) + f.sum(
            -1
        )  # mean for the static dft / sum for tddft

    def functional_value(self, z: torch.Tensor):
        output = self.model(z)[:, 0, :]
...
class TDDFTadiabaticModel_xxzzxz(nn.Module):
    def __init__(
        self,
        n_conv_layers: int = None,
        in_features: int = None,
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        x = torch.squeeze(x)
        # +-1 outputs
        x = 4 * torch.sin(x)  # commented for the magn2field
        # x = torch.sigmoid(x)  # we want to prove the Cross Entropy
        return x
...
class TDDFTadiabaticModelNoNorm(nn.Module):
    def __init__(
        self,
        n_conv_layers: int = None,
        in_features: int = None,
