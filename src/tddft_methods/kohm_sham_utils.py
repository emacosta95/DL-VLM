...
def compute_the_magnetization(psi: torch.Tensor) -> Tuple[torch.DoubleTensor]:
    ...
    x = torch.einsum("li,ij,lj->l", psi.conj(), x_operator, psi).double()
    z = torch.einsum("li,ij,lj->l", psi.conj(), z_operator, psi).double()
...
def get_the_hamiltonian(psi: torch.ComplexType, h: torch.Tensor, energy: nn.Module):
    x, z = compute_the_magnetization(psi=psi)
    ...
    hamiltonian = build_hamiltonian(field_x=-1 * omega_eff[0], field_z=-1 * h_eff[0])
...
def time_step_crank_nicolson_algorithm(
    ...
    for j in range(self_consistent_steps):
        hamiltonian, eng0 = get_the_hamiltonian(psi=psi, h=h, energy=energy)
        psi_1 = crank_nicolson_algorithm(hamiltonian=hamiltonian, psi=psi, dt=dt)

        hamiltonian_1, eng = get_the_hamiltonian(psi=psi_1, h=h_plus, energy=energy)
        psi = crank_nicolson_algorithm(
            hamiltonian=0.5 * (hamiltonian_1 + hamiltonian), psi=psi, dt=dt
        )
