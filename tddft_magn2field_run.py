...
from src.tddft_methods.kohm_sham_utils import (
    compute_the_gradient_magn2field,
    build_hamiltonian,
...
model = torch.load(
    "model_rep/kohm_sham/magn2field/Magn2Field_reduction_2_input_channel_dataset_h_0.0-2.0_omega_0.0-2.0_j_1_1nn_n_500k_unet_l_train_8_[40, 40, 40, 40, 40, 40]_hc_5_ks_1_ps_6_nconv_0_nblock",
    map_location="cpu",
)
model.eval()
model = model.to(dtype=torch.double)

model_energy = torch.load(
...
model_energy.eval()
model_energy = model_energy.to(dtype=torch.double)
energy = Energy_XXZX(model_energy)
...
energy = Energy_XXZX(model=model_energy)
...
for q, rate in enumerate(rates):
    ...
    for i in trange(time.shape[0]):
        ...
        if i == len(time) - 1:
            ...
            field_eff = compute_the_gradient_magn2field(
                m=m0, h=h[i].unsqueeze(0), model=model
            )
            h_eff = field_eff[:, 0]
            omega_eff = field_eff[:, 1]
        ...
        if periodic:
            np.savez(
                f"data/kohm_sham_approach/results/magn2field/tddft_periodic_uniform_magn2field_h_0_2_omega_0_2_ti_0_tf_{tf:.0f}_hi_{hi[0,0].item():.4f}_delta_{delta[0,0].item():.4f}_omegai_{hi[1,0].item():.1f}_delta_{delta[1,0].item():.1f}_steps_{steps}_self_consistent_steps_{self_consistent_step}_ndata_{ndata}_exp_{exponent_algorithm}",
                x_qutip=x_qutip_tot,
                z_qutip=z_qutip_tot,
                z=z_tot,
                x=x_tot,
            ...
            np.savez(
                f"data/kohm_sham_approach/results/magn2field/tddft_quench_uniform_magn2field_h_0_2_omega_0_2_ti_0_tf_{tf:.0f}_hi_{hi[0,0].item():.4f}_hf_{hf[0,0].item():.4f}_omegai_{hi[1,0].item():.1f}_omegaf_{hf[1,0].item():.1f}_steps_{steps}_self_consistent_steps_{self_consistent_step}_ndata_{ndata}_exp_{exponent_algorithm}",
                x_qutip=x_qutip_tot,
                z_qutip=z_qutip_tot,
                z=z_tot,
                x=x_tot,
