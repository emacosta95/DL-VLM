time_interval=48




nohup python train.py --time_interval=$time_interval --model_type="Seq2Seq" --input_channels=1 --kernel_size 5 1 --data_path 'data/uniform/train_size_6_tf_10.0_dt_0.1_sigma_1_9_c_0_4_set_of_gaussians_100_n_dataset_15000.npz' --pooling=1 --model_name=seq2seq/uniform_gaussian_noise/regularization_1_sigma_1_9_c_0_4_set_of_gaussians_100_t_01_l_6_nt_$time_interval\_15k_pbc --hidden_channels 80 80 80 80      --epochs=3000      > output_uniform_seq2seq_80_hc_15k_48_nt_regularization_1.txt &


# nohup python train.py --time_interval=$time_interval --model_type="LSTM" --n_layers=5 --input_size=6 --data_path 'data/periodic/train_size_6_tf_10.0_dt_0.1_a_0_1_omega_0_4_n_dataset_150000.npz' --pooling=1 --model_name=lstm_periodic_omega_4_amplitude_1_t_01_l_6_mixed_nt_$time_interval\_150k_pbc --hidden_channels 40       --epochs=3000      > output_tddft_test_lstm_$time_interval\_15k.txt &

#seq2seq_regularization_01_sigma_1_9_c_0_4_set_of_gaussians_100_t_01_l_6_nt_$time_interval\_15k_pbc